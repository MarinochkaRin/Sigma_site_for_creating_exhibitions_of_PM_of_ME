...
/*    @GetMapping("/user/profile/{userId}")
    ...
    public String registerUser(@RequestParam("photoId") MultipartFile file, @RequestParam Map<String, String> body, @RequestParam("role") String role, User user, RedirectAttributes redirectAttributes) {
        if (role.equals("ADMIN")) {
            ...
            userService.createOrganizer(body.get("username"), body.get("password"),body.get("photoId"), body.get("firstName"), body.get("secondName") , body.get("locationPerson"));
            redirectAttributes.addFlashAttribute("message", "organizer_created");
        } else if (role.equals("MODELLER")) {
            userService.createModeller(body.get("username"), body.get("password"),body.get("photoId"), body.get("firstName"), body.get("secondName") , body.get("locationPerson"));
            redirectAttributes.addFlashAttribute("message", "modeller_created");
        } else {
            String filename = UUIDHelper.random();
            user.setPhotoId(filename);
            userService.createUser(body.get("username"), body.get("password"),filename, body.get("firstName"), body.get("secondName") , body.get("locationPerson"));
